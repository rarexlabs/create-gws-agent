...
5. After the user attaches the file, move it to `credentials/google-oauth-client.json`, creating `credentials` if needed. Restrict both paths to the current user (`0700` for the directory and `0600` for the file on macOS and Linux), and do not accept a symbolic link at either path. Handle the filename and location without reading, parsing, or displaying the file's contents.
